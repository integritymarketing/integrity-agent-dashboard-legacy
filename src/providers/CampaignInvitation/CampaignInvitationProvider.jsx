...
export const CampaignInvitationProvider = ({ children }) => {
  ...
  const EMAIL_URL = `${import.meta.env.VITE_LEADS_URL}/api/v2.0/Campaign/Email`;
  const TEXT_URL = `${import.meta.env.VITE_LEADS_URL}/api/v2.0/Campaign/sms`;
  const AGENT_PURL_URL = `${
    import.meta.env.VITE_AGENTS_URL
  }/api/v1.0/Purl/npn/${npn}`;
  const AGENT_ACCOUNT_DETAILS_URL = `${
    import.meta.env.VITE_AGENTS_URL
  }/api/v1.0/AgentMobile/Available/${agentId}`;
  const CREATE_CAMPAIGN_URL = `${
    import.meta.env.VITE_COMMUNICATION_API
  }/CampaignLog/Create`;
  const UPDATE_CAMPAIGN_URL = `${
    import.meta.env.VITE_COMMUNICATION_API
  }/CampaignLog/Update`;
  const GET_CAMPAIGN_URL = `${
    import.meta.env.VITE_COMMUNICATION_API
  }/CampaignLog`;
  ...
  useEffect(() => {
    const filterCondition =
      ...
      filteredCount >= 0;
    handleCampaignAction(filterCondition, {
  ...
  const { fetchTableDataWithoutFilters, fetchTableData } =
  ...
  const handleTemplateData = (data, initial) => {
    ...
    const action = data?.campaignActions?.find(
      item =>
        (item?.actionFilter || item?.actionDescription) &&
        item?.actionOrder === 1
    );
    if (initial) {
      if (action) {
  ...
  useEffect(() => {
    if (
      !isAdvancedMode &&
      allCampaignActions?.length &&
      campaignChannel === 'Email'
    ) {
      const basicCampaignActions = allCampaignActions?.filter(
        action => action?.actionType === 'Basic'
      );
    ...
    async templateId => {
      try {
        ...
        if (resData?.length) {
          ...
          setAllCampaignInvitationData(sortedCampaigns);
          if (templateId) {
            const templateData = resData?.find(
  ...
  const handleCreateCampaignFromContact = async ({
    ...
    if (campaignChannel === 'Email') {
      ...
      const templateData = emailCampaignsData?.find(
        item => item?.campaignName === customCampaign
      );
    ...
    if (campaignChannel === 'Sms') {
      ...
      const templateData = textCampaignsData?.find(
        item => item?.campaignName === customCampaign
      );
          ...
          if (resData?.campaignChannel === 'Email') {
            ...
            if (templateData) {
              ...
              if (
                ...
                if (resData?.campaignSelectedAction === 'a contact when') {
                  const filteredContentStatus =
                    customFilterData?.filteredContentStatus || '';

                  handleSummaryBarInfo([], filteredContentStatus, 0);
                }
                if (
                  resData?.campaignSelectedAction === 'contacts filtered by…'
                ) {
                  const filterData = await fetchTableData({
                    ...
                    selectedFilterSections:
                      customFilterData?.selectedFilterSections || [],
                    isSilent: true,
                    returnAll: true,
  ...
  const getAgentAccountInformation = useCallback(async () => {
    try {
      ...
      return {
        newAgentAccountDetails: agentAccountDetailsData,
        newAgentPurlURL: agentPurlURLData,
      };
    } catch (error) {
      Sentry.captureException(error);
      showToast({
  ...
  const updateCampaignRequestPayload = async ({
    campaign_Status,
    template_Id,
    campaign_Channel,
    campaign_ActionType,
    ...
    let newAgentAccountDetails;
    let newAgentPurlURL;

    if (!agentAccountDetails || !agentPurlURL) {
      const response = await getAgentAccountInformation();
      newAgentAccountDetails = response?.newAgentAccountDetails;
      newAgentPurlURL = response?.newAgentPurlURL;
    }

    const latestAgentAccountDetails =
      newAgentAccountDetails || agentAccountDetails;
    const latestAgentPurlURL = newAgentPurlURL || agentPurlURL;

    const formattedPhoneNumber =
      latestAgentAccountDetails?.agentVirtualPhoneNumber?.replace(/^\+1/, '');
    ...
    const payload = {
      ...
      agentNpn: npn,
      campaignType:
        actionType === 'a contact when'
          ? 'Event'
          : channel === 'Sms' || actionType === 'a contact'
          ? 'Individual'
          : 'Blast',
      ...
      requestPayload: {
        ...
        custom1: `${import.meta.env.VITE_MEDICARE_ENROLL}/quick-profile?purl=${
          latestAgentPurlURL?.agentPurlCode
        }`,
        custom2: 'Integrity',
        custom3: formattedPhoneNumber,
        custom4: `${import.meta.env.VITE_MEDICARE_ENROLL}/?purl=${
          latestAgentPurlURL?.agentPurlCode
        }`,
        custom5: latestAgentAccountDetails?.caLicense,
        custom6: latestAgentAccountDetails?.profileImageUrl,
        custom7: `${firstName.charAt(0)} ${lastName.charAt(0)}`,
        custom8: null,
      },
    };
    if (professionalInfo?.agentOfficeLocation?.agencyName) {
      payload.requestPayload.custom8 =
        professionalInfo?.agentOfficeLocation?.agencyName;
    } else {
    ...
    if (payload?.campaignSelectedAction === 'all contacts') {
      ...
      const striginfiedId = id?.toString();
      ...
      return {
        ...
        eventTrigger: {
          ...
          when: triggerType === 'stage' ? 'Status' : striginfiedId,
          becomes: triggerType === 'stage' ? striginfiedId : '',
        },
        requestPayload: {
          ...payload.requestPayload,
          leads: [],
  ...
  const handleCreateOrUpdateCampaign = useCallback(
    async ({
      ...
      try {
        let resData = null;
        if (isUpdate) {
          const requestPayload = await updateCampaignRequestPayload({
            campaign_Status,
            template_Id,
            campaign_Channel,
            campaign_ActionType,
          ...
          resData = await updateCampaign(requestPayload, false);
        } else {
          const requestPayload =
            payload ?? createCampaignRequestPayload(campaign_Status);
  ...
  return (
    <CampaignInvitationContext.Provider value={getContextValue()}>
      {children}
    </CampaignInvitationContext.Provider>
  );
