...
const SaveToContact = ({
  ...
  const handlelingNavWithCallBack = useCallback(
    response => {
      ...
      if (page === 'plans page accumulation') {
        navWithCallBack(`/life/iul-accumulation/${leadId}`, response);
      }
      if (page === 'plans page protection') {
        navWithCallBack(`/life/iul-protection/${leadId}`, response);
      }
