...
const PolicyCard = ({ callData }) => {
  ...
  const isHealthPolicy = Boolean(
    healthPolicyTypes.some(
      type => type?.toLowerCase() == callData?.productType?.toLowerCase()
    )
  );

  const status = callData?.policyStatus || '';
  const policyStatus =
    status === 'terminated' ? 'Inactive' : capitalizeFirstLetter(status);
  const IconComponent = isHealthPolicy
    ? healthIcons[policyStatus]
    : lifeIcons[policyStatus];
