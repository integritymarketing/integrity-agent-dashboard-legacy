...
const FinalExpensePlansResultContainer = () => {
  ...
  const [quoteOptionsInfoModalOpen, setQuoteOptionsInfoModalOpen] =
    useState(false);
  ...
  const { Get: getAgentNonRTS, loading: getAgentNonRTSLoading } = useFetch(
  ...
  const [coverageAmount, setCoverageAmount] = usePreferences(
    defaultCoverageAmount,
    'coverage'
  );
  const [monthlyPremiumAmount, setMonthlyPremiumAmount] = usePreferences(
    defaultMonthlyPremium,
    'monthlyPremium'
  );
  const [coverageType, setCoverageType] = usePreferences(
    COVERAGE_TYPE[4].value,
    'sessionCoverageType'
  );
  ...
  const [appointmentSession, setAppointmentSession] = usePreferences(
    false,
    'appointmentSession'
  );
  const [isShowExcludedProducts, setIsShowExcludedProducts] = usePreferences(
    false,
    'sessionIsShowExcludedProducts'
  );
  const [isShowAlternativeProducts, setIsShowAlternativeProducts] =
  ...
  const handleMyAppointedProductsCheck = useCallback(
    (isChecked = false) => {
      ...
      setIsMyAppointedProducts(isChecked);
      if (isChecked) {
        setIsShowAlternativeProducts(false);
      }
    },
    [isRTS, isMyAppointedProducts, setIsShowAlternativeProducts]
  );

  const handleIsShowExcludedProductsCheck = useCallback(
    (isChecked = false) => {
      setIsShowExcludedProducts(isChecked);
      if (isChecked) {
        setIsShowAlternativeProducts(false);
      }
    },
    [isShowExcludedProducts, setIsShowAlternativeProducts]
  );

  const handleIsShowAlternativeProductsCheck = useCallback(
    (isChecked = false) => {
      setIsShowAlternativeProducts(isChecked);
      if (isChecked) {
        setIsMyAppointedProducts(false);
        setIsShowExcludedProducts(false);
      }
    ...
    [
      setIsShowAlternativeProducts,
      setIsMyAppointedProducts,
      setIsShowExcludedProducts,
    ]
  );
  ...
  return (
    <>
      ...
      <div
        className={`${styles.contentWrapper} ${isMobile ? styles.column : ''}`}
      >
          ...
          <CurrencyAdjuster
            ...
            disableMin={
              selectedTab === COVERAGE_AMOUNT
                ...
                : monthlyPremiumAmount <= min
            }
            disableMax={
              selectedTab === COVERAGE_AMOUNT
                ? coverageAmount >= covMax
                : monthlyPremiumAmount >= max
            }
            increment={increment}
            decrement={decrement}
        ...
        {!getAgentNonRTSLoading && (
          <PlanDetailsContainer
            ...
            isRTS={isRTS}
            setIsRTS={setIsRTS}
            isShowAlternativeProducts={isShowAlternativeProducts}
            handleIsShowAlternativeProductsCheck={
              handleIsShowAlternativeProductsCheck
            }
            healthConditions={healthConditions}
