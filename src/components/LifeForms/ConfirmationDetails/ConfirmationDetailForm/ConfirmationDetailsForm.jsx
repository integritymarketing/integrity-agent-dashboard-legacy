...
import {
  FinalExpenseIntakeForm,
  FinalExpenseIntakeFormMobileStep0,
  FinalExpenseIntakeFormMobileStep1,
} from 'schemas';
...
export const ConfirmationDetailsForm = ({ contactId, onSave, quoteType }) => {
  ...
  const onSaveHealthInfo = useCallback(async () => {
    ...
    const updatedFormData = {
      ...
      height: values.feet
        ? Number(values.feet * 12) + Number(values.feet)
        : null,
  ...
  useEffect(() => {
    if (leadDetails) {
      ...
      const code = sessionItem
        ? JSON.parse(sessionItem).stateCode
        : leadDetails?.addresses?.[0]?.stateCode;

      const hFeet = leadDetails?.height
        ? Math.floor(leadDetails?.height / 12)
        : '';
      ...
      GENDER_OPTS.map(option => (
        <div
          ...
          className={`${styles.optionValueBox} ${
            option.value === values.gender ? styles.selected : ''
          }`}
    ...
    () =>
      SMOKER_OPTS.map(option => (
        <div
          key={option.label}
          className={`${styles.optionValueBox} ${
            option.value === values.isTobaccoUser ? styles.selected : ''
          }`}
    ...
    <div className={styles.finalExpenseContactDetailsForm}>
      ...
      <div className={styles.contactCard}>
        <div className={styles.contactCardHeading}>
          ...
          {(!mobileStepNumber || mobileStepNumber === 0) && (
            <p>{CONTACT_FORM_SUBTITLE}</p>
          )}
        </div>
