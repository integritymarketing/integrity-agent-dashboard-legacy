const HealthInactive = () => {
    return (
        <svg
            id="Policy_Status_Circle_Health"
            data-name="Policy Status Circle – Health"
            xmlns="http://www.w3.org/2000/svg"
            width="44"
            height="44"
            viewBox="0 0 44 44"
        >
            <g id="IconCircleBG" transform="translate(0 4)">
                <circle id="Ellipse_299" data-name="Ellipse 299" cx="20" cy="20" r="20" fill="#f1f1f1" />
            </g>
            <g id="Health_Policy_None" data-name="Health Policy None" transform="translate(8 12)">
                <g id="Health_-_Cross" data-name="Health - Cross">
                    <g id="Button_-_BG" data-name="Button - BG">
                        <rect id="bg" width="24" height="24" fill="#ff1717" opacity="0" />
                    </g>
                    <path
                        id="Cross"
                        d="M17.271,6.729V1.778A1.278,1.278,0,0,0,15.993.5H8.005A1.278,1.278,0,0,0,6.727,1.778V6.729H1.777A1.277,1.277,0,0,0,.5,8.007v7.988a1.277,1.277,0,0,0,1.277,1.278H6.727v4.951A1.277,1.277,0,0,0,8.005,23.5h7.988a1.277,1.277,0,0,0,1.278-1.276V17.273h4.951A1.278,1.278,0,0,0,23.5,15.995V8.007a1.278,1.278,0,0,0-1.278-1.278Z"
                        fill="none"
                        stroke="#ccc"
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        strokeWidth="1"
                    />
                </g>
            </g>
            <g id="StatusCount" transform="translate(28)" opacity="0">
                <g id="Ellipse_720" data-name="Ellipse 720" fill="#f1faff" stroke="#fff" strokeWidth="1">
                    <circle cx="8" cy="8" r="8" stroke="none" />
                    <circle cx="8" cy="8" r="7.5" fill="none" />
                </g>
                <text
                    id="_3"
                    data-name="3"
                    transform="translate(4 13)"
                    fill="#4178ff"
                    fontSize="13"
                    fontFamily="Lato-Regular, Lato"
                >
                    <tspan x="0" y="0">
                        3
                    </tspan>
                </text>
            </g>
        </svg>
    );
};

export default HealthInactive;
