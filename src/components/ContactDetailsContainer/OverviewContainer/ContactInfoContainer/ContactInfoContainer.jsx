...
export const ContactInfoContainer = ({ isMobile }) => {
  ...
  const leadPhone = useMemo(() => {
    return phonesData?.[0]?.leadPhone
      ? formatPhoneNumber(phonesData?.[0]?.leadPhone)
      : '';
  }, [phonesData]);
  ...
  const leadMBID = useMemo(() => {
    return medicareBeneficiaryID
      ? formatMBID(medicareBeneficiaryID)
      : NOT_AVAILABLE;
  }, [medicareBeneficiaryID]);

  const leadMBIDFull = useMemo(() => {
    return medicareBeneficiaryID
      ? formatMBID(medicareBeneficiaryID, true)
      : NOT_AVAILABLE;
  }, [medicareBeneficiaryID]);
  ...
  return (
    <WithLoader isLoading={isLoadingLeadDetails}>
      <Box marginTop={'20px'}>
        <Box
          className={`${styles.horizontalLayout} ${styles.gap}  ${
            isCollapsed ? styles.underLine : ''
          } `}
          marginBottom='16px'
        >
          <Box className={styles.iconWithTitle}>
            ...
            <Label
              value={isEditMode ? 'Edit Contact Details' : 'Contact Details'}
            />
          </Box>
          {!isEditMode && (
            <Box
        ...
        {!isEditMode && (!isCollapsed || !isMobile) && (
          <Box>
            ...
            <SectionContainer>
              <Label value='Primary Contact' color='#717171' size='14px' />
              <Label
                value={
                  (isPrimary === 'email' && 'Email') ||
                  (isPrimary === 'phone' && 'Phone')
                }
                color='#052A63'
                size='20px'
              />
            ...
            <SectionContainer>
              <Label
                value='Medicare Beneficiary Identifier (MBI)'
                color='#717171'
                size='14px'
              />
              <div className={styles.beneficiaryId}>
                ...
                />
                <span
                  onClick={() => setShowBeneficiaryId(!isShowBeneficiaryId)}
                >
              ...
              <Box
                className={`${styles.horizontalLayout} ${styles.dateAlignment}`}
              >
                <Box>
                  <Label
                    value='Part A start date'
                    color='#717171'
                    size='14px'
                  />
            ...
            <SectionContainer>
              <Paper className={styles.specialAssistanceCard} elevation='0'>
                <Stack
                  className={styles.specialAssistanceContainer}
                  direction='row'
                >
                  <Typography
                    className={styles.specialAssistanceHeader}
                    variant='custom'
                  >
                    Special Assistance
                  </Typography>
            ...
            >
              <Label
                value={`Created Date: ${leadCreatedDate}`}
                color='#717171'
                size='14px'
              />
            </Box>
