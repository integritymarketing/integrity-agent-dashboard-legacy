...
export const QUOTES_API_NEW_VERSION = 'v2.0';
...
export class ClientsService {
  ...
  getCampaignLeads = async (
    ...
    const response = await this._clientAPIRequest(
      `${
        import.meta.env.VITE_LEADS_URL
      }/api/v2.0/Campaign/GetCampaignLeads/${campaignId}/${actionOrderedId}`,
      'POST',
      body
    );
    if (response?.status >= 400) {
  ...
  bulkExportContacts = async reqData => {
    const response = await this._clientAPIRequest(
      `${
        import.meta.env.VITE_LEADS_URL
      }/api/${LEADS_API_VERSION}/Leads/bulkexport`,
      'POST',
      reqData
    );
  ...
  reActivateClients = async data => {
    const response = await this._clientAPIRequest(
      `${
        import.meta.env.VITE_LEADS_URL
      }/api/${LEADS_API_VERSION}/Leads/reactivate`,
      'PUT',
      data
    );
  ...
  getStatuses = async () => {
    const response = await this._clientAPIRequest(
      `${
        import.meta.env.VITE_LEADS_URL
      }/api/${LEADS_STATUS_API_VERSION}/Leads/statuses`
    );
  ...
  deleteReminder = async id => {
    const response = await this._clientAPIRequest(
      `${
        import.meta.env.VITE_LEADS_URL
      }/api/${LEADS_API_VERSION}/Reminders/${id}`,
      'DELETE'
    );
  ...
  updateActivity = async (data, id) => {
    const response = await this._clientAPIRequest(
      `${
        import.meta.env.VITE_LEADS_URL
      }/api/${LEADS_API_VERSION}/Activities/${id}`,
      'PUT',
      data
    );
  ...
  getDuplicateContact = async contact => {
    ...
    const response = await this._clientAPIRequest(
      `${
        import.meta.env.VITE_LEADS_URL
      }/api/${LEADS_API_VERSION}/Leads/GetDuplicateContact`,
      'POST',
      reqData
    );
    return response;
  ...
  addNewContact = async contact => {
    ...
    let requestData = {
      ...
      medicareBeneficiaryID: medicareBeneficiaryID
        ? flattenMBI(medicareBeneficiaryID)
        : undefined,
      partA: partA ? formatServerDate(partA) : null,
      partB: partB ? formatServerDate(partB) : null,
    };

    requestData.primaryCommunication =
      primaryCommunication || (email ? 'email' : 'phone');
  ...
  getContactPreferences = async id => {
    const response = await this._clientAPIRequest(
      `${
        import.meta.env.VITE_LEADS_URL
      }/api/${LEADS_API_VERSION}/ContactPreferences/${id}`,
      'GET'
    );
  ...
  getLeadPharmacies = async leadId => {
    const response = await this._clientAPIRequest(
      `${
        import.meta.env.VITE_QUOTE_URL
      }/api/${QUOTES_API_VERSION}/Lead/${leadId}/Pharmacies`,
      'GET'
    );
  ...
  getLeadPrescriptions = async leadId => {
    const response = await this._clientAPIRequest(
      `${
        import.meta.env.VITE_QUOTE_URL
      }/api/${QUOTES_API_VERSION}/Lead/${leadId}/Prescriptions`,
      'GET'
    );
    return response?.json().then(res => res || []);
  };
  ...
  createPrescription = async (leadId, reqData, consumerId) => {
    ...
    if (consumerId) {
      buildUrl = `${
        import.meta.env.VITE_QUOTE_URL
      }/api/${QUOTES_API_VERSION}/Lead/${leadId}/Prescriptions/syncid/${consumerId}`;
    }
    const response = await this._clientAPIRequest(
      `${buildUrl}`,
      'POST',
  ...
  deletePrescription = async (leadId, id, consumerId) => {
    let buildUrl = `${
      import.meta.env.VITE_QUOTE_URL
    }/api/${QUOTES_API_VERSION}/Lead/${leadId}/Prescriptions/${id}`;
    if (consumerId) {
      buildUrl = `${
        import.meta.env.VITE_QUOTE_URL
      }/api/${QUOTES_API_VERSION}/Lead/${leadId}/Prescriptions/${id}/${consumerId}`;
    }
  ...
  getDrugNames = async drugName => {
    const response = await this._clientAPIRequest(
      `${
        import.meta.env.VITE_QUOTE_URL
      }/api/${QUOTES_API_VERSION}/Search/DrugName?drugName=${drugName}`,
      'GET'
    );
    return response?.json();
  };
  ...
  getDrugDetails = async drugID => {
    const response = await this._clientAPIRequest(
      `${
        import.meta.env.VITE_QUOTE_URL
      }/api/${QUOTES_API_VERSION}/Search/DrugDetail?id=${drugID}`,
      'GET'
    );
    return response?.json();
  };
  ...
  deleteHealthCondition = async (contactId, conditionId) => {
    const buildUrl = `${
      import.meta.env.VITE_QUOTE_URL
    }/api/${QUOTES_API_VERSION}/HealthCondition/Lead/${contactId}/id/${conditionId}`;
    const response = await this._clientAPIRequest(`${buildUrl}`, 'DELETE');
    return response;
  };
  ...
  deletePharmacy = async (leadId, id, consumerId) => {
    let buildUrl = `${
      import.meta.env.VITE_QUOTE_URL
    }/api/${QUOTES_API_VERSION}/Lead/${leadId}/Pharmacies/${id}`;
    if (consumerId) {
      buildUrl = `${
        import.meta.env.VITE_QUOTE_URL
      }/api/${QUOTES_API_VERSION}/Lead/${leadId}/Pharmacies/${id}/${consumerId}`;
    }
    const response = await this._clientAPIRequest(`${buildUrl}`, 'DELETE');
  ...
  createPharmacy = async (leadId, reqData, consumerId) => {
    ...
    if (consumerId) {
      buildUrl = `${
        import.meta.env.VITE_QUOTE_URL
      }/api/${QUOTES_API_VERSION}/Lead/${leadId}/Pharmacies/${consumerId}`;
    }
    const response = await this._clientAPIRequest(
      `${buildUrl}`,
      'POST',
  ...
  getLeadProviders = async leadId => {
    const response = await this._clientAPIRequest(
      `${
        import.meta.env.VITE_QUOTE_URL
      }/api/${QUOTES_API_VERSION}/Lead/${leadId}/Provider/ProviderSearchLookup`,
      'GET'
    );
  ...
  createLeadProvider = async (leadId, data, consumerId) => {
    let buildUrl = `${
      import.meta.env.VITE_QUOTE_URL
    }/api/${QUOTES_API_VERSION}/Lead/${leadId}/Provider`;
    if (consumerId) {
      buildUrl = `${
        import.meta.env.VITE_QUOTE_URL
      }/api/${QUOTES_API_VERSION}/Lead/${leadId}/Provider/${consumerId}`;
    }
  ...
  searchProviders = async query => {
    const response = await this._clientAPIRequest(
      `${
        import.meta.env.VITE_QUOTE_URL
      }/api/${QUOTES_API_VERSION}/Search/Providers?${query}`,
      'GET'
    );
  ...
  searchPharmacies = async (payload, isOnline = false) => {
    if (isOnline) {
      const response = await this._clientAPIRequest(
        `${
          import.meta.env.VITE_QUOTE_URL
        }/api/${QUOTES_API_VERSION}/Search/DigitalPharmacies`,
        'GET'
      );
      ...
      const response = await this._clientAPIRequest(
        `${
          import.meta.env.VITE_QUOTE_URL
        }/api/${QUOTES_API_VERSION}/Search/Pharmacies`,
        'POST',
        payload
      );
  ...
  getLatlongByAddress = async (zipcode, address) => {
    ...
    const response = await fetch(
      `https://api.mapbox.com/geocoding/v5/mapbox.places/${address} ${zipcode}.json?limit=1&types=address&access_token=${
        import.meta.env.VITE_MAPBOX_GEO_TOKEN
      }
        `,
      opts
    );
  ...
  getSoaByLinkCode = async (leadsId, linkCode) => {
    const response = await this._clientAPIRequest(
      `${
        import.meta.env.VITE_LEADS_URL
      }/api/v2.0/lead/${leadsId}/Soa/${linkCode}`,
      'GET'
    );
  ...
  getDashbaordSummary = async () => {
    const response = await this._clientAPIRequest(
      `${
        import.meta.env.VITE_LEADS_URL
      }/api/${LEADS_STATUS_API_VERSION}/Leads/summary`,
      'GET'
    );
  ...
  getApplicationCount = async sortByRange => {
    ...
    const response = await this._clientAPIRequest(
      `${
        import.meta.env.VITE_LEADS_URL
      }/api/${LEADS_API_VERSION}/Leads/appCount?startdate=${startDate}&enddate=${endDate}`,
      'GET'
    );
  ...
  getAgents = async npn => {
    const response = await this._clientAPIRequest(
      ...
      }/api/${AGENTS_API_VERSION}/Agents/rts/${npn}`,
      'GET'
    );
  ...
  getAgentPurlCodeByNPN = async agentnpn => {
    const response = await this._clientAPIRequest(
      `${
        import.meta.env.VITE_AGENTS_URL
      }/api/${AGENTS_API_VERSION}/Purl/npn/${agentnpn}`,
      'GET'
    );
    return response?.json();
  };
  ...
  updateAgentAvailability = async payload => {
    const url = `${
      import.meta.env.VITE_AGENTS_URL
    }/api/${AGENTS_API_VERSION}/AgentMobile/Availability`;
  ...
  updateAgentPreferences = async payload => {
    const response = await this._clientAPIRequest(
      `${
        import.meta.env.VITE_AGENTS_URL
      }/api/${AGENTS_API_VERSION}/AgentMobile/Preference`,
      'POST',
      payload
    );
    if (response?.ok) {
  ...
  getAgentAvailability = async id => {
    const response = await this._clientAPIRequest(
      `${
        import.meta.env.VITE_AGENTS_URL
      }/api/${AGENTS_API_VERSION}/AgentMobile/Available/${id}`,
      'GET'
    );
    if (response?.ok) {
      return response?.json();
  ...
  updateAgentCallForwardingNumber = async payload => {
    const response = await this._clientAPIRequest(
      `${
        import.meta.env.VITE_AGENTS_URL
      }/api/${AGENTS_API_VERSION}/AgentMobile/CallForwardNumber`,
      'POST',
      payload
    );
    if (response?.ok) {
  ...
  getAgentByAgentId = async id => {
    const response = await this._clientAPIRequest(
      `${
        import.meta.env.VITE_AGENTS_URL
      }/api/${AGENTS_API_VERSION}/Agents/${id}`,
      'GET'
    );
  ...
  generateAgentTwiloNumber = async id => {
    await this._clientAPIRequest(
      `${
        import.meta.env.VITE_AGENTS_URL
      }/api/${AGENTS_API_VERSION}/Call/GenerateVirtualPhoneNumber/${id}?limit=1`,
      'POST'
    );
  };
  ...
  getAllTagsByGroups = async () => {
    const response = await this._clientAPIRequest(
      `${
        import.meta.env.VITE_LEADS_URL
      }/api/${LEADS_API_VERSION}/Tag/TagsGroupByCategory?mappedLeadTagsOnly=true`,
      'GET'
    );
    return response?.json();
  };
  ...
  getTagsGroupByCategory = async () => {
    const response = await this._clientAPIRequest(
      `${
        import.meta.env.VITE_LEADS_URL
      }/api/${LEADS_API_VERSION}/Tag/TagsGroupByCategory`,
      'GET'
    );
    return response?.json();
  };
  ...
  getTaskList = async (npn, dateRange, status) => {
    const response = await this._clientAPIRequest(
      `${
        import.meta.env.VITE_LEADS_URL
      }/api/v2.0/Leads/tasks/${npn}/${dateRange}/${status}`,
      'GET'
    );
  ...
  getTaskListCount = async (npn, dateRange) => {
    const response = await this._clientAPIRequest(
      `${
        import.meta.env.VITE_LEADS_URL
      }/api/v3.0/Leads/taskcount/${npn}/${dateRange}`,
      'GET'
    );
