...
export default function LinkToContact() {
  ...
  const callIsAssigned =
    callRecordings?.length === 0
      ? false
      : !callRecordings.some(
          callRecording => callRecording?.callLogId === parseInt(callLogId)
        );

  const callRecordItem = callRecordings?.filter(
    callRecording => callRecording?.callLogId === parseInt(callLogId)
  );
  const callType = callRecordItem?.[0]?.callType
    ? callRecordItem?.[0]?.callType
    : 'inbound';
  ...
  const extractAndFlattenTags = callLogs => {
    ...
    const uniqueTags = Array.from(
      new Map(flattenedTags.map(tag => [tag.tagId, tag])).values()
    );
  ...
  const flattenedTags =
    callsRecordingTags?.length > 0
      ? extractAndFlattenTags(callsRecordingTags)
      : [];
  ...
  const goToAddNewContactsPage = useCallback(() => {
    ...
    const newQueryParams = [tagsParam, callFromParam, inboundParam, nameParam]
      .filter(Boolean)
      .join('&');
    const route = `${baseRoute}${logIdParam}${
          ...
          {callIsAssigned && callerName !== 'Text' ? (
            <div className={styles.medContent}>
              <div className={styles.content}>
                This call is already assigned to a lead.
              </div>
            </div>
          ) : (
            <Box>
