...
const useActivities = ({ getLeadDetails }) => {
    ...
    const [activities, leadFullName, activitiesLength] = useRecoilValue(state.selectors.contactLeadActivitiesSelector);

    const [open, setOpen] = useRecoilState(state.atoms.addNewActivitiesAtom);
    const [editActivity, setEditActivity] = useState(null);
    const [selectedActivity, setSelectedActivity] = useState(null);

    const pageHasMoreRows = useRecoilValue(state.selectors.pageHasMoreRowsSelector);
    const filterValues = useRecoilValue(state.selectors.filterValuesSelector);
    const [activitiesSortBy, setActivitiesSortBy] = useRecoilState(state.atoms.activitiesSortingByDateAtom);
    const [activitiesPageLimit, setActivitiesPageLimit] = useRecoilState(state.atoms.activitiesPageLimitAtom);
    const [activitiesFilters, setActivitiesFilters] = useRecoilState(state.atoms.activitiesFiltersAtom);
    ...
    const onResetFilter = useCallback(() => {
        setActivitiesFilters(Object.fromEntries(filterValues.map((filter) => [filter.name, false])));
