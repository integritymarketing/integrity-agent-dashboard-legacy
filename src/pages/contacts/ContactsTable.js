...
function Table({
    ...
    const { getTableProps, getTableBodyProps, headerGroups, prepareRow, page, pageCount, selectedFlatRows } = useTable(
        {
            columns,
            data,
                        ...
                        <tr {...headerGroup.getHeaderGroupProps()}>
                            {headerGroup.headers.map((column) => (
                                <th {...column.getHeaderProps()}>
                                    <div className={styles["tb-headers"]}>{column.render("Header")}</div>
                                </th>
                            ))}
                        </tr>
                    ))}
                                ...
                                {row.cells.map((cell) => {
                                    return (
                                        <td {...cell.getCellProps()}>
                                            <div className={styles["tb-cell"]}>{cell.render("Cell")}</div>
                                        </td>
                                    );
                                })}
                            </tr>
...
function ContactsTable({
            ...
            clientsService
                ...
                .then((list) => {
                    const listData = list.result.map((res) => ({
                        ...res,
                        contactRecordType:
                            res.contactRecordType === ("Client" || "client") && !res.statusName
                                ? "Enrolled"
    ...
    useEffect(() => {
        ...
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [filteredLeadIdsLength, pageIndex, pageSize, searchString, sort, applyFilters]);
    ...
    const columns = useMemo(
        () => [
                ...
                Cell: ({ value, row }) => {
                    ...
                    return (
                        <div className={styles.contactPersonNameWrapper}>
                            <Link to={`/contact/${row.original.leadsId}`} className={styles.contactPersonName}>
                                {name}
                            </Link>
            ...
            },
            {
                ...
                accessor: "statusName",
                Cell: ({ value, row }) => {
                    return <StageSelect initialValue={value} originalData={row.original} refreshData={handleRefresh} />;
                },
            },
            {
                Header: "Reminder",
            ...
            },
            {
                ...
                accessor: (row) => {
                    return <div className={styles.primaryContact}>{getPrimaryContact(row)}</div>;
                },
            },
            {
                Header: "",
    ...
    const mobile_columns = useMemo(
        () => [
            ...
            },
            {
                ...
                accessor: "statusName",
                Cell: ({ value, row }) => {
                    const data = statusOptions.filter((x) => x?.value === value)?.[0] || undefined;
                    if (data) {
                        return <ColorOptionRender {...data} filter={true} className={styles.stageOption} />;
                    } else {
                        return value;
                    }
                },
    ...
    return (
        <>
            ...
            {openModal && <SOAModal id={leadId} openSOAModal={openModal} setOpenSOAModal={setOpenModal} />}
            {leadConnectModal && (
                <ConnectModal
