    ...
    return (<Card>
        <div>
            <div className={styles.cardHeader}>
                <div className={`pt-1 pb-1 hdg hdg--4 text-truncate ${styles.contactName} ${displayName !== 'Unnamed Contact' ? "" : "text-muted"}`} >
                    {displayName}
                </div>
                <div>
                    <Link to={`/contact/${client.leadsId}`} className={styles.viewLink}> View </Link>
                </div>
            </div>
            <div className={styles.cardInfo}>
                <div>
                    <label className={styles.cardInfoLabel} htmlFor={`stage-${client.leadsId}`}>Statge</label>
                    <Select
                ...
                <div className={styles.reminder}>
                    <label className={styles.cardInfoLabel} htmlFor={`reminder-${client.leadsId}`}>Reminder</label>
                    <Textfield
            ...
            <div className={styles.primaryContactHeader}>
                Primary Contact
            </div>
            <div className={styles.primaryContactInfo}>
                {primaryContact}
            </div>
        </div>
    </Card>)
...
function ContactsCard({ searchString, sort }) {
    ...
    const handleChangeStage = async (client, val) => {
        try {
            await clientsService.updateClient(
                client,
                { ...client, leadStatusId: allStatuses.find(status => status.statusName === val)?.leadStatusId }
            );
            fetchData({
                pageSize,
    ...
    return (<Container className="mt-scale-3" style={{ paddingLeft: 0 }}>
        {data.length === 0 && (
