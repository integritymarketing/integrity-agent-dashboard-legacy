...
function useFetchTableData() {
    ...
    const applyFilters = useMemo(() => {
        return {
            ...
            stages: queryParams.get("Stage") ? queryParams.get("Stage").split(",") : [],
    ...
    const fetchTableDataWithoutFilters = useCallback(
        async ({ pageIndex, pageSize, sort, searchString, returnAll }) => {
            try {
                ...
                showToast({ type: "error", message: "Failed to fetch table data", time: 5000 });
    ...
    const fetchTableData = useCallback(
        async ({
            ...
            try {
                ...
                const leadIds = filterLeadIds || duplicateIds || campaignsLeadIds || null;
                let response;
                ...
                if (appendData) {
                    ...
                    setAllLeads((prev) => [...prev, ...listData.map((contact) => contact.leadsId)]);
                } else {
                ...
                setIsLoading(false);
                Sentry.captureException(error);
                showToast({ type: "error", message: "Failed to load data", time: 10000 });
            }
        },
        [applyFilters, clientsService, showToast]
    );

    return { tableData, isLoading, fetchTableData, fetchTableDataWithoutFilters, allLeads, pageResult };
}
