...
// Restricts input to number characters only
...
export const getShoppersColorScheme = title => {
  const priority = title?.includes('1')
    ? '1'
    : title?.includes('2')
    ? '2'
    : '3';
...
const blackWordStyle =
  'color: #434a51; font-size: 16px; font-style: italic; margin-right: 4px;';
const blueWordStyle =
  'color: #052a63; font-weight: 600; font-size: 16px; margin-right: 4px;';
const normalStyle =
  'color: #717171; font-weight: 400; font-size: 16px; margin-left: 4px;';
...
export const styleEventDescription = (description, isNormal) => {
  ...
  return output
    ...
    .map(word => {
      ...
      if (blackWords.includes(cleanWord.toLowerCase())) {
        return `<span style="${
          isNormal ? normalStyle : blackWordStyle
        }">${word}</span>`;
      } else {
        return `<span style="${
          isNormal ? normalStyle : blueWordStyle
        }">${word}</span>`;
      }
    })
    .join(' ');
